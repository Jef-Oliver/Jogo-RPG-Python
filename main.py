...
def exibir_npc(npc):
    print(
        f"Nome: {npc['nome']} // Level: {npc['level']
                                         } // Dano: {npc['dano']} // HP: {npc['hp']} // EXP: {npc['exp']}"
    )


def exibir_player():
    print(
        f"Nome: {player['nome']} // Level: {player['level']} // Dano: {player['dano']} // HP: {
            player['hp']}/{player['hp_max']} // EXP: {player['exp']}/{player['exp_max']}"
    )
...
def level_up():
    if player['exp'] >= player['exp_max']:
        ...
        player['hp_max'] += 20
        print(f"Parabéns! Você subiu para o level {player['level']}!")
